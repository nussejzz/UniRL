...
from unirl.models.bagel.vendor.modeling.qwen2.configuration_qwen2 import Qwen2Config as _Qwen2Config
from unirl.models.bagel.vendor.modeling.cache_utils.taylorseer import (
...
class PackedAttentionMoT(Qwen2Attention):
    ...
    def forward_inference(
        ...
        if mode == 'und':
            ...
            # MIGRATION grad-safety edit (the ONE non-pristine change; see vendor/__init__.py):
            # write into a FRESH tensor instead of inplace-indexing packed_attn_output, which
            # is a view of the flash_attn custom Function output — inplace modification of such
            # a view is forbidden under autograd (fine only under no_grad, which is why the
            # diffuse<->replay ratio test passed but RL replay backprop crashed:
            # "Output 0 of ViewBackward0 is a view and is being modified inplace"). The und
